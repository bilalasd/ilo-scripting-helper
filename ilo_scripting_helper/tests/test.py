...
class TestIloScriptingTools(unittest.TestCase):
    def test_read_values_from_file(self):
        ...
        with self.subTest("file doesn't exist"):
            try:
                values = ilo_scripting_helper.read_in_values_from_file(
                    os.path.join(os.path.dirname(__file__), "asd.txt")
                )
                self.fail()
    ...
    def test_extract_ip_from_string(self):
        ...
        for case in cases:
            with self.subTest(case["ip"]):
                result = ilo_scripting_helper.extract_ip_from_string(case["ip"])
    ...
    def test_ilo_session_class_attributes(self):
        for case in cases:
            with self.subTest(case["ip"]):
                ...
                self.assertFalse(
                    session.ip != case["clean_ip"],
                    session.ip + " != " + case["clean_ip"],
                )
                self.assertFalse(
                    session.full_url != case["url_https"],
                    session.full_url + " != " + case["url_https"],
                )
                self.assertFalse(
                    session.API_url != case["url_API_https"],
                    session.API_url + " != " + case["url_API_https"],
                )
                self.assertFalse(
                    session.model != case["model"],
                    session.model + " != " + case["model"],
                )
                self.assertFalse(
                    session.full_ilo_version_string != case["full_ilo_version"],
                    session.full_ilo_version_string + " != " + case["full_ilo_version"],
                )
                self.assertFalse(
                    session.ilo_version != case["ilo_version"],
                    session.ilo_version.name + " != " + case["ilo_version"].name,
                )
                self.assertFalse(
                    session.ilo_firmware_version != case["ilo_firmware_version"],
                    session.ilo_firmware_version
                    + " != "
                    + case["ilo_firmware_version"],
                )

    def test_environment_info_windows(self):
        self.assertFalse(
            "Windows-10-10.0.18363-SP0"
            != ilo_scripting_helper.EnvironmentInfo.get_system_platform_full_name(),
            "Windows-10-10.0.18363-SP0"
            + "!="
            + ilo_scripting_helper.EnvironmentInfo.get_system_platform_full_name(),
        )
        self.assertFalse(
            ilo_scripting_helper.EnvironmentInfo.OS.WINDOWS
            != ilo_scripting_helper.EnvironmentInfo.get_system_platform(),
            ilo_scripting_helper.EnvironmentInfo.OS.WINDOWS.name
            + " != "
            + ilo_scripting_helper.EnvironmentInfo.get_system_platform().name,
        )
    ...
    def test_environment_info_mac(self):
        ...
        self.assertFalse(
            ilo_scripting_helper.EnvironmentInfo.OS.DARWIN
            != ilo_scripting_helper.EnvironmentInfo.get_system_platform(),
            ilo_scripting_helper.EnvironmentInfo.OS.DARWIN.name
            + " != "
            + ilo_scripting_helper.EnvironmentInfo.get_system_platform().name,
        )
